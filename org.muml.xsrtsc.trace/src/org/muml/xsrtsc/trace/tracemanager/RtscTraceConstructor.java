...
public class RtscTraceConstructor implements ITraceConstructor {
	...
	public void addStep(fr.inria.diverse.trace.commons.model.trace.Step step) {
		...
		if (step != null && step instanceof rtscTrace.Steps.SpecificStep) {
			...
			if (step_cast instanceof rtscTrace.Steps.Rtsc_Realtimestatechart_InitRTSC) {
				rtscTrace.Steps.Rtsc_Realtimestatechart_InitRTSC rtsc_Realtimestatechart_InitRTSCInstance = (rtscTrace.Steps.Rtsc_Realtimestatechart_InitRTSC) step_cast;
				traceRoot.getRtsc_Realtimestatechart_InitRTSC_Sequence().add(rtsc_Realtimestatechart_InitRTSCInstance);
			} else if (step_cast instanceof rtscTrace.Steps.Rtsc_Realtimestatechart_Initialize) {
	...
	private void addImplicitStep(rtscTrace.Steps.SpecificStep currentStep, rtscTrace.States.State startingState,
		...
		if (currentStep instanceof rtscTrace.Steps.Rtsc_Realtimestatechart_Initialize) {
			implicitStep = rtscTrace.Steps.StepsFactory.eINSTANCE
					.createRtsc_Realtimestatechart_Initialize_ImplicitStep();
		} else if (currentStep instanceof rtscTrace.Steps.Rtsc_Realtimestatechart_Step) {
