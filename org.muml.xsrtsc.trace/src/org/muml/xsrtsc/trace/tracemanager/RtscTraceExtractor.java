...
public class RtscTraceExtractor implements ITraceExtractor {

	private rtscTrace.SpecificTrace traceRoot;

	final private List<List<? extends rtscTrace.States.Value>> valueTraces = Collections
			.synchronizedList(new ArrayList<>());

	private List<rtscTrace.States.State> statesTrace;

	final private Map<rtscTrace.Steps.SpecificStep, Integer> stepToStartingIndex = new HashMap<>();
	final private Map<rtscTrace.Steps.SpecificStep, Integer> stepToEndingIndex = new HashMap<>();
	...
	private final List<rtscTrace.States.Value> observedValues = new ArrayList<>();

	public RtscTraceExtractor() {
	...
	private ValueWrapper getValueWrapper(rtscTrace.States.Value value, int valueIndex) {
		List<rtscTrace.States.State> states = value.getStatesNoOpposite();
		rtscTrace.States.State firstState = states.get(0);
	...
	private rtscTrace.States.Value getValueAt(int traceIndex, int stateIndex) {
		rtscTrace.States.Value result = null;
		if (traceIndex >= 0 && traceIndex < valueTraces.size()) {
			final List<? extends rtscTrace.States.Value> valueTrace = valueTraces.get(traceIndex);
			final rtscTrace.States.State state = statesTrace.get(stateIndex);
	...
	public ValueWrapper getValueWrapper(int traceIndex, int stateIndex) {
		rtscTrace.States.Value value = getValueAt(traceIndex, stateIndex);
		if (value == null) {
			return null;
		}
		List<rtscTrace.States.State> states = value.getStatesNoOpposite();
	...
	private List<Integer> computeStateComparisonList(List<rtscTrace.States.Value> values) {
		final List<Integer> valueIndexes = new ArrayList<>();
		for (int i = 0; i < values.size(); i++) {
			final rtscTrace.States.Value value = values.get(i);
			int idx = -1;
	...
	private void updateEquivalenceClasses(rtscTrace.States.State state) {
		final List<rtscTrace.States.Value> values = getAllStateValues(state, true);
	...
	private void updateEquivalenceClasses(List<rtscTrace.States.State> states) {
	...
	public boolean compareStates(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final rtscTrace.States.State state1;
		final rtscTrace.States.State state2;

		if (eObject1 instanceof rtscTrace.States.State) {
			state1 = (rtscTrace.States.State) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof rtscTrace.States.State) {
			state2 = (rtscTrace.States.State) eObject2;
		} else {
			return false;
		}

		final List<rtscTrace.States.Value> values1 = getAllStateValues(state1);
		final List<rtscTrace.States.Value> values2 = getAllStateValues(state2);
		...
		for (int i = 0; i < values1.size(); i++) {
			if (!respectIgnored || !isValueTraceIgnored(i)) {
				final rtscTrace.States.Value value1 = values1.get(i);
				final rtscTrace.States.Value value2 = values2.get(i);
				if (value1 != value2) {
	...
	public boolean compareSteps(EObject eObject1, EObject eObject2) {
		final rtscTrace.Steps.SpecificStep step1;
		final rtscTrace.Steps.SpecificStep step2;

		if (eObject1 instanceof rtscTrace.Steps.SpecificStep) {
			step1 = (rtscTrace.Steps.SpecificStep) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof rtscTrace.Steps.SpecificStep) {
			step2 = (rtscTrace.Steps.SpecificStep) eObject2;
		} else {
			return false;
		}
	...
	public boolean compareStatesWithSteps(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		...
		if (compareStates(state1, state2, respectIgnored)) {
			final List<rtscTrace.Steps.SpecificStep> endedSteps1 = state1.getEndedSteps();
			final List<rtscTrace.Steps.SpecificStep> startedSteps1 = state1.getStartedSteps();
			final List<rtscTrace.Steps.SpecificStep> endedSteps2 = state2.getEndedSteps();
			final List<rtscTrace.Steps.SpecificStep> startedSteps2 = state2.getStartedSteps();
			if (endedSteps1.size() == endedSteps2.size() && startedSteps1.size() == startedSteps2.size()) {
				boolean result = true;
				for (int i = 0; i < endedSteps1.size(); i++) {
					final rtscTrace.Steps.SpecificStep endedStep1 = endedSteps1.get(i);
					final rtscTrace.Steps.SpecificStep endedStep2 = endedSteps2.get(i);
				...
				for (int i = 0; i < startedSteps1.size(); i++) {
					final rtscTrace.Steps.SpecificStep startedStep1 = startedSteps1.get(i);
					final rtscTrace.Steps.SpecificStep startedStep2 = startedSteps2.get(i);
	...
	public boolean compareTraces(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final rtscTrace.SpecificTrace trace1;
		final rtscTrace.SpecificTrace trace2;

		if (eObject1 instanceof rtscTrace.SpecificTrace) {
			trace1 = (rtscTrace.SpecificTrace) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof rtscTrace.SpecificTrace) {
			trace2 = (rtscTrace.SpecificTrace) eObject2;
		} else {
			return false;
		}

		final List<rtscTrace.States.State> states1 = trace1.getStatesTrace();
		final List<rtscTrace.States.State> states2 = trace2.getStatesTrace();
		...
		for (int i = 0; i < states1.size(); i++) {
			final rtscTrace.States.State state1 = states1.get(i);
			final rtscTrace.States.State state2 = states2.get(i);
	...
	private List<rtscTrace.States.Value> getAllStateValues(rtscTrace.States.State state) {
		return getAllStateValues(state, false);
	}

	private List<rtscTrace.States.Value> getAllStateValues(rtscTrace.States.State state, boolean includeHiddenValues) {
		final List<rtscTrace.States.Value> result = new ArrayList<>();
		for (int i = 0; i < valueTraces.size(); i++) {
			if (includeHiddenValues || !isValueTraceIgnored(i)) {
				final List<? extends rtscTrace.States.Value> trace = valueTraces.get(i);
				boolean notFound = true;
				for (rtscTrace.States.Value value : trace) {
					if (value.getStatesNoOpposite().contains(state)) {
						result.add(value);
						notFound = false;
						break;
	...
	private boolean isStateBreakable(rtscTrace.States.State state) {
		final boolean b = state.getStartedSteps().size() == 1;
		if (b) {
			rtscTrace.Steps.SpecificStep s = state.getStartedSteps().get(0);
			return !(s instanceof rtscTrace.Steps.Rtsc_Realtimestatechart_Initialize_ImplicitStep
					|| s instanceof rtscTrace.Steps.Rtsc_Realtimestatechart_Step_ImplicitStep);
		}
		return true;
	}
	...
	public StateWrapper getStateWrapper(EObject state) {
		if (state instanceof rtscTrace.States.State) {
			final int idx = statesTrace.indexOf(state);
			if (idx != -1) {
				final rtscTrace.States.State state_cast = (rtscTrace.States.State) state;
	...
	public List<StateWrapper> getStateWrappers(int start, int end) {
		...
		for (int i = startStateIndex; i < endStateIndex + 1; i++) {
			final rtscTrace.States.State state = statesTrace.get(i);
	...
	public List<ValueWrapper> getValueWrappers(int valueTraceIndex, int start, int end) {
		...
		if (valueTraceIndex < valueTraces.size()) {
			final List<? extends rtscTrace.States.Value> valueTrace = valueTraces.get(valueTraceIndex);
			for (rtscTrace.States.Value value : valueTrace) {
	...
	public StepWrapper getStepWrapper(Step step) {
		if (step instanceof rtscTrace.Steps.SpecificStep) {
			final rtscTrace.Steps.SpecificStep step_cast = (rtscTrace.Steps.SpecificStep) step;
			...
			if (step_cast instanceof SequentialStep<?>) {
				subSteps.addAll(((SequentialStep<rtscTrace.Steps.SpecificStep>) step_cast).getSubSteps());
			}
	...
	public int getValuesTraceLength(int traceIndex) {
		if (traceIndex > -1 && traceIndex < valueTraces.size()) {
			List<? extends rtscTrace.States.Value> trace = valueTraces.get(traceIndex);
			return trace.size();
		}
		return -1;
	}
	...
	public void statesAdded(List<EObject> states) {
		updateEquivalenceClasses(states.stream().map(e -> (rtscTrace.States.State) e).collect(Collectors.toList()));
		notifyListeners();
	}
